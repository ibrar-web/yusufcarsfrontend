...
export default function AdminCategoryItemsPage() {
  ...
  const [categoryItems, setCategoryItems] = useState<AdminCategory[]>([]);
  ...
  const fethCategoryItems = useCallback(async (categoryId: string) => {
    ...
    try {
      const response = await apiGet<CategoryItemsResponse>(
        apiRoutes.admin.categoryItems.list(categoryId)
      );
  ...
  useEffect(() => {
    if (subCategoryId) {
      ...
      fethCategoryItems(subCategoryId);
    }
  }, [subCategoryId, fethCategoryItems]);

  const filteredSubcategories = useMemo(() => {
    const term = subcategorySearch.trim().toLowerCase();
    if (!term) return categoryItems;
    return categoryItems.filter((categoryItems) =>
      `${categoryItems.name} ${categoryItems.id}`.toLowerCase().includes(term)
    );
  }, [subcategorySearch, categoryItems]);
  ...
  const handleCreateCategoryItem = async () => {
    ...
    try {
      ...
      const response = await apiPost<SubcategoryCreateResponse>(
        apiRoutes.admin.categoryItems.create(subCategoryId),
        payload
      );
      ...
      setCategoryItems((prev) => [createdSubcategory, ...prev]);
      resetForm();
