...
export function VehicleSummaryCard({
  ...
  const vehicleExtras = [
    ...
    { label: "CO2 Emissions", value: vehicle?.co2Emissions },
    {
      ...
      value: vehicle?.engineCapacity ? `${vehicle.engineCapacity}cc` : undefined,
    },
  ].filter(
    (detail) =>
      ...
      <div className="relative z-10 px-6 sm:px-12 py-10">
        {(title || subtitle) && (
          <div className="text-center mb-6">
            ...
            {subtitle && (
              <p className="font-['Roboto'] text-white/60 mt-2">{subtitle}</p>
            )}
          </div>
        )}

        <div className="relative overflow-hidden bg-gradient-to-br from-[#1E293B]/60 to-[#0F172A]/70 backdrop-blur-xl rounded-2xl p-8 border border-white/5">
        ...
        <p className="relative z-10 text-center mt-4 font-['Roboto'] text-sm text-white/60">
